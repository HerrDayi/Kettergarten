...
Ralf Kletter, Geschäftsführer`
    ...
    {
        id: 6,
        title: "Level 6: Die große TÜV-Sicherheitsabnahme",
        theme: "Lagebeziehung von Geraden & LGS",
        email: {
            sender: "TÜV Rheinland - Sachverständigenbüro <abnahme@tuv-rheinland.de>",
            subject: "Prüfprotokoll / Kreuzungsprüfung der Hauptseile AB und CD",
            date: "20. Juni 2026",
            body: `Sehr geehrte Damen und Herren,

im Rahmen der endgültigen Sicherheitsabnahme müssen wir die Betriebssicherheit der Hauptseile AB und CD prüfen. Es darf auf keinen Fall zu einer physischen Überschneidung der Seilbahnen kommen, da dies lebensgefährlich wäre.

Plattform D wurde mittlerweile bei $D(8 \\mid 2 \\mid 1)$ errichtet (somit haben wir Seil AB von A nach B und Seil CD von C nach D).

Bitte reichen Sie uns die vollständige Lagebeziehungs-Berechnung ein. Setzen Sie dazu die Geraden der beiden Seile gleich ($g(r) = h(s)$). Berechnen Sie die Parameter $r$ und $s$ aus den ersten beiden Gleichungen und prüfen Sie das Ergebnis in der dritten Gleichung (Z-Koordinate).

Geben Sie an, ob sich die Seile mathematisch schneiden oder windschief sind, und ob die Seilbahnen betriebssicher sind (Sicherheitsabstand mindestens $1{,}5\\text{ m}$).

Mit freundlichen Grüßen,
Dipl.-Ing. Marcus Vektor, Prüfsachverständiger`
        },
        taskDescription: "Berechne die Parameter $r$ und $s$ durch Gleichsetzen der Geradengleichungen für Seil AB und Seil CD, prüfe diese in Gleichung III und bestimme die Lagebeziehung.",
        setup: {
            platforms: [
                { name: 'A', x: 2, y: 3, z: 2 },
                { name: 'B', x: 8, y: 2, z: 4 },
        ...
        target: {
            ...
            s: 1,
            relation: 'windschief',
            safe: true // distance is ~2.8m, which is safe (>1.5m)
        },
        inputs: [
            { id: 'param_r', label: 'Parameter r (aus Gl. I & II)', type: 'number', expected: 1 },
            { id: 'param_s', label: 'Parameter s (aus Gl. I & II)', type: 'number', expected: 1 },
            { id: 'lgs_result', label: 'Ist die Überprüfung in Gleichung III erfolgreich?', type: 'select', options: [
                { value: '', label: '-- Bitte wählen --' },
                { value: 'consistent', label: 'Ja (LHS = RHS) -> Geraden schneiden sich' },
                { value: 'contradiction', label: 'Nein (LHS != RHS) -> Geraden sind windschief' }
            ], expected: 'contradiction' },
            { id: 'safety_check', label: 'TÜV-Sicherheitsfreigabe erteilen?', type: 'select', options: [
                { value: '', label: '-- Bitte wählen --' },
                { value: 'safe', label: 'Freigabe erteilt (Windschief & Abstand >= 1,5m)' },
                { value: 'unsafe', label: 'Keine Freigabe (Kollision oder Abstand < 1,5m)' }
            ], expected: 'safe' }
        ],
        helpCards: [
            {
                title: "Lagebeziehung untersuchen (LGS)",
                content: "1. **Gleichsetzen**: Setze $g(r) = h(s)$ an.\n2. **LGS aufstellen**:\n   (I) $a_1 + r \\cdot u_1 = b_1 + s \\cdot w_1$\n   (II) $a_2 + r \\cdot u_2 = b_2 + s \\cdot w_2$\n   (III) $a_3 + r \\cdot u_3 = b_3 + s \\cdot w_3$\n3. **Lösen**: Berechne $r$ und $s$ aus (I) und (II).\n4. **Prüfen**: Setze $r$ und $s$ in (III) ein. Entsteht eine wahre Aussage, schneiden sich die Geraden. Entsteht ein Widerspruch, sind sie windschief."
            },
            {
                title: "Kreuzungsprodukt & Abstand",
                content: "Der Abstand zweier windschiefer Geraden mit Stützpunkten $P_1, P_2$ und Richtungsvektoren $\\vec{u}, \\vec{w}$ berechnet sich durch:\n$$d = \\frac{|(\\vec{p}_2 - \\vec{p}_1) \\cdot (\\vec{u} \\times \\vec{w})|}{|\\vec{u} \\times \\vec{w}|}$$"
            }
        ]
    }
];
